...
// ─── Male milestone overrides ─────────────────────────────────────────────────
// 17 character milestones at tier boundaries + mid-tier picks (~21% of 81).
// Names are public-domain: Russian folklore, mythology, history, Soviet films.
// L0 (Дохляк) keeps its archetype name — sprite is regenerated as the anchor.
NAMES_AND_DESCRIPTIONS[4]  = ["Колобок",         "От бабушки ушёл. От штанги — пока нет."];
NAMES_AND_DESCRIPTIONS[9]  = ["Левша",           "Подковал блоху. Гриф подковать — следующий шаг."];
NAMES_AND_DESCRIPTIONS[13] = ["Иванушка",        "С печи слез — к штанге пошёл."];
NAMES_AND_DESCRIPTIONS[18] = ["Мюнхгаузен",      "Сам себя за волосы вытащил. Штангу — тоже вытащу."];
NAMES_AND_DESCRIPTIONS[22] = ["Геракл-юноша",    "Двенадцать подвигов впереди. Пока — двенадцать повторов."];
NAMES_AND_DESCRIPTIONS[27] = ["Спартак",         "Я — Спартак. И жим это подтвердит."];
NAMES_AND_DESCRIPTIONS[31] = ["Тесей",           "Лабиринт зала пройден. Минотавр повержен."];
NAMES_AND_DESCRIPTIONS[36] = ["Святогор",        "Сила земли в твоих ладонях."];
NAMES_AND_DESCRIPTIONS[40] = ["Самсон",          "Волосы стрижёшь — а сила всё растёт."];
NAMES_AND_DESCRIPTIONS[45] = ["Добрыня Никитич", "Богатырь. Второй из трёх. Первый уже рядом."];
NAMES_AND_DESCRIPTIONS[49] = ["Ахилл",           "Пятка прикрыта. Жим — открыт."];
NAMES_AND_DESCRIPTIONS[54] = ["Алёша Попович",   "Хитрость подкреплена силой."];
NAMES_AND_DESCRIPTIONS[58] = ["Тор",             "Молот при себе. Гриф — тоже."];
NAMES_AND_DESCRIPTIONS[63] = ["Илья Муромец",    "33 года ждал. Зал наконец открылся."];
NAMES_AND_DESCRIPTIONS[72] = ["Зевс",            "С Олимпа — прямо за блинами 25 кг."];
NAMES_AND_DESCRIPTIONS[80] = ["Атлас",           "Держишь небо. Штанга — для разминки."];

// ─── Female level names ───────────────────────────────────────────────────────
// Same shape as the male list: 81 entries, ~80% archetype progression names
// (in feminine grammatical form) and ~20% character milestones (★ marked) at
// the same level indices used for the male side: L0, L4, L9, L13, L18, L22,
// L27, L31, L36, L40, L45, L49, L54, L58, L63, L72, L80. All character names
// are public-domain.
const NAMES_AND_DESCRIPTIONS_FEMALE: Array<[string, string]> = [
  // Tier 0 — L0–8
  ["Феечка",            "Не поднимала ничего тяжелее помады."],          // ★ L0
  ["Хрупкая",           "Бутылка воды кажется тебе гантелью."],
  ["Спичка",            "Ветер тебя качает с ноги на ногу."],
  ["Тоненькая",         "Ремень делает на тебе три витка."],
  ["Золушка",           "Швабру отложила. Гриф взяла."],                  // ★ L4
  ["Стесняшка",         "Зашла в зал — спряталась в углу."],
  ["Доходяжка",         "Дверь в зал кажется чугунной."],
  ["Веточка",           "Тебя путают с вешалкой для одежды."],
  ["Цыпочка",           "Кудахчешь над пустым грифом."],
  // Tier 1 — L9–17
  ["Снегурочка",        "В зале не тает — но пока зябко."],               // ★ L9
  ["Зелёненькая",       "Ходишь по залу как первоклашка в музее."],
  ["Любопытная",        "Уже знаешь, где раздевалка."],
  ["Туристка",          "Заходишь в зал как на экскурсию."],
  ["Машенька",          "Медведя не боишься. Штангу — пока да."],         // ★ L13
  ["Школьница",         "Делаешь подходы по 5 раз с пустым грифом."],
  ["Кардио-девочка",    "Думаешь, дорожка заменит силовые. Ошибаешься."],
  ["Помощница",         "Уже не падаешь под штангой. Прогресс."],
  ["Подопечная",        "Тренер всё ещё стоит рядом на всякий случай."],
  // Tier 2 — L18–26
  ["Афродита",          "Из пены морской — сразу к гантелям."],           // ★ L18
  ["Стажёрка",          "Знаешь разницу между жимом и тягой."],
  ["Регулярная",        "Ходишь в зал чаще, чем за молоком."],
  ["Самоучка",          "Учишь технику по роликам в телефоне."],
  ["Геба",              "Юная богиня кубок подняла. Гриф — следующий."],  // ★ L22
  ["Кандидатка",        "Гриф уже не кажется страшным."],
  ["Дисциплинированная","День ног не пропускаешь. Почти."],
  ["Постоянница",       "На ресепшене узнают тебя по имени."],
  ["Активистка",        "Кардио после силовой — иногда."],
  // Tier 3 — L27–35
  ["Клеопатра",         "Царство в зале уже отвоёвано."],                 // ★ L27
  ["Юниорка",           "Можешь подсказать новенькой, как держать гантель."],
  ["Спортивная",        "Форма уже заметна. Чуть-чуть."],
  ["Железная леди",     "Шейкер всегда с тобой."],
  ["Бастет",            "Кошачья грация на каждом подходе."],             // ★ L31
  ["Завсегдатая",       "Знаешь, где лежат блины 20 кг."],
  ["Тренирующаяся",     "Можешь объяснить, что такое суперсет."],
  ["Уверенная",         "Поднимаешь без страха травмы."],
  ["Опытная",           "Ты лучше большинства в зале."],
  // Tier 4 — L36–44
  ["Брунгильда",        "Валькирия в малом масштабе."],                   // ★ L36
  ["Прогрессирующая",   "Веса растут каждую неделю."],
  ["Тренированная",     "По плечам видно, что ходишь в зал."],
  ["Бывалая",           "Знаешь, зачем нужен пояс."],
  ["Морриган",          "Вороны кружат — штанга трясётся."],              // ★ L40
  ["Атлетка",           "На входе кивают как своей."],
  ["Силачка",           "Жмёшь свой вес. Это уважают."],
  ["Бодифитнес-леди",   "Считаешь белки в граммах."],
  ["Мощная",            "Топ натягивается на дельтах."],
  // Tier 5 — L45–53
  ["Жанна д'Арк",       "Ведёшь зал в бой. Без лишних слов."],            // ★ L45
  ["Жилистая",          "Сила ощущается в каждом движении."],
  ["Крепкая",           "Под топом видно работу."],
  ["Подтянутая",        "В зеркало смотришь с одобрением."],
  ["Томирис",           "Кира победила. Штангу — тоже."],                 // ★ L49
  ["Сложенная",         "На пляже просят показать пресс."],
  ["Прокачанная",       "Молодёжь спрашивает у тебя совет."],
  ["Сильная",           "Жмёшь в полтора своих веса."],
  ["Воительница",       "В зале как у себя дома."],
  // Tier 6 — L54–62
  ["Поляница",          "Русская богатырша. Слово держит."],              // ★ L54
  ["Жимовичка",         "Жим лёжа — твоё призвание."],
  ["Тяговичка",         "Становая — твоя страсть."],
  ["Силовичка",         "Готова к соревнованиям."],
  ["Марья Моревна",     "Кощея в подвале держала. Тренажёры — тоже."],    // ★ L58
  ["Мускулистая",       "Спина закрывает половину зеркала."],
  ["Богатырша-стажёрка","Чувствуется потенциал."],
  ["Жим-машина",        "Повторы — как на конвейере."],
  ["Силовая атлетка",   "Веса, что пугали, теперь — твоя разминка."],
  // Tier 7 — L63–71
  ["Валькирия",         "Забирает лучших. В зале — тебя."],               // ★ L63
  ["Грозная",           "На тебя оборачиваются в зале."],
  ["Богатырша",         "Ты — пример для подражания."],
  ["Гора мышц",         "Двери становятся узкими."],
  ["Громила",           "Стулья жалобно скрипят."],
  ["Танкистка",         "Просто проходишь сквозь толпу."],
  ["Зверюга",           "Рычишь под штангой. Все молчат."],
  ["Колоссиха",         "Размеры впечатляют опытных."],
  ["Глыба",             "Тебе уступают дорогу."],
  // Tier 8 — L72–80
  ["Гайя",              "Земля носит — ты поднимаешь."],                  // ★ L72
  ["Мастерица",         "Твоя техника безупречна."],
  ["Профи",             "Молодёжь смотрит как на легенду."],
  ["Сильнейшая",        "В твоём зале нет тебе равных."],
  ["Чемпионка зала",    "Все рекорды — твои."],
  ["Тяжеловесница",     "Цифры на штанге впечатляют."],
  ["Гранитная",         "Тренеры консультируются у тебя."],
  ["Несгибаемая",       "Каждая тренировка — битва с собой."],
  ["Легенда",           "Твоё имя будут помнить века."],                  // ★ L80
];
