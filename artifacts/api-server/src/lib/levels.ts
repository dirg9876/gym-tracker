...
// Replace 8 generic names with famous characters at tier-boundary levels.
NAMES_AND_DESCRIPTIONS[9]  = ["Шурик",          "Операция Ы засчитана. Приступаем к тренировкам."];
NAMES_AND_DESCRIPTIONS[18] = ["Электроник",      "Я не могу остановиться — это прогресс."];
NAMES_AND_DESCRIPTIONS[27] = ["Спартак",         "Я — Спартак. И жим это подтвердит."];
NAMES_AND_DESCRIPTIONS[36] = ["Терминатор",      "Я вернусь. После разминки."];
...
NAMES_AND_DESCRIPTIONS[72] = ["Супермен",        "Плащ не нужен — силы и так хватает."];

// ─── Female level names ───────────────────────────────────────────────────────
const NAMES_AND_DESCRIPTIONS_FEMALE: Array<[string, string]> = [
  // Tier 0 — L0–8: Fairy-tale girls (PD)
  ["Дюймовочка",        "Помещаешься в цветочный горшок. Пока."],
  ["Алиса",             "Зелье 'выпей меня' не помогло — штанга всё равно больше."],
  ["Алёнушка",          "Ищешь братца. Нашла гриф."],
  ["Красная Шапочка",   "Корзинку несёшь — мышцы не спешат."],
  ["Золушка",           "Швабру отложила. Гриф взяла."],
  ["Белоснежка",        "Семь гномов смотрят с завистью."],
  ["Спящая красавица",  "На тренировку — с третьего будильника."],
  ["Рапунцель",         "Косу заплела, в зал спустилась."],
  ["Гретель",           "Ведьму победила. Теперь штангу."],
  // Tier 1 — L9–17: Russian/Slavic fairy-tale girls (PD)
  ["Снегурочка",        "В зале не тает, но мёрзнет."],
  ["Герда",             "Ищет своего Кая среди гантелей."],
  ["Варвара-краса",     "Коса длинная, результаты — пока нет."],
  ["Марьюшка",          "Ходит по залу как по полю."],
  ["Настенька",         "Подружилась с тренажёром-чудовищем."],
  ["Несмеяна",          "Улыбнулась после первого жима."],
  ["Машенька",          "Медведя в зале не боится."],
  ["Пастушка",          "Овец пересчитала — теперь повторы."],
  ["Ткачиха",           "Сила в руках уже видна."],
  // Tier 2 — L18–26: Greek mythology, getting serious (PD)
  ["Психея",            "Душа крепнет вместе с телом."],
  ["Гера",              "Царица зала, хоть и без трона."],
  ["Афродита",          "Из пены морской — в тренажёрку."],
  ["Деметра",           "Урожай результатов уже виден."],
  ["Геба",              "Юная богиня кубок поднимает уверенно."],
  ["Ирида",             "Радуга прогресса видна каждому."],
  ["Эвридика",          "Из ада тренировок выходит без оглядки."],
  ["Пандора",           "Открыла ящик — там штанга."],
  ["Кассандра",         "Свои PR-ы предсказывает заранее."],
  // Tier 3 — L27–35: Egyptian and Eastern queens (PD)
  ["Нефертити",         "Профиль с фрески, осанка — атлета."],
  ["Клеопатра",         "Царство в зале уже отвоёвано."],
  ["Бастет",            "Кошачья грация на каждом подходе."],
  ["Исида",             "Магия — в технике."],
  ["Хатшепсут",         "Фараон-женщина. И ещё какой."],
  ["Иштар",             "Богиня-воин из древнего Шумера."],
  ["Семирамида",        "Висячие сады? Нет, висячий пресс."],
  ["Шахерезада",        "Каждая тренировка — новая история."],
  ["Мулан",             "Императорские нормативы пройдены."],
  // Tier 4 — L36–44: Norse and Celtic warriors (PD)
  ["Брунгильда",        "Валькирия в малом."],
  ["Сигрюн",            "Северный ветер слабее твоего жима."],
  ["Фрейя",             "Половину павших забирает Один, лучшую — она."],
  ["Скади",             "Лыжи и охота — её разминка."],
  ["Боудикка",          "Колесницы боятся, штанга — нет."],
  ["Морриган",          "Ворон на плече, штанга в руках."],
  ["Хель",              "Половина зала её боится."],
  ["Грид",              "Великанша, помогавшая Тору."],
  ["Нанна",             "Верная своему делу до конца."],
  // Tier 5 — L45–53: Historical warrior queens (PD)
  ["Жанна д'Арк",       "Ведёшь зал в бой без слов."],
  ["Княгиня Ольга",     "Не древлянам мстит — день ног объявляет."],
  ["Артемисия",         "Командовала флотом, теперь — штангой."],
  ["Зенобия",           "Восток покорила, зал — следом."],
  ["Томирис",           "Кира Великого победила. Штангу — тоже."],
  ["Аталанта",          "Золотые яблоки её не остановили."],
  ["Ипполита",          "Пояс силы — её атрибут."],
  ["Пентесилея",        "Троянцы боялись её в бою."],
  ["Камилла",           "Бегала по воде, поднимает железо."],
  // Tier 6 — L54–62: Russian bogatyr heroines (PD)
  ["Поляница",          "Русская богатырша. Слово держит."],
  ["Настасья Микулишна","Дочь Микулы — сильнее мужей."],
  ["Василиса Микулишна","Мужа из плена вызволила одной хитростью и силой."],
  ["Марья Моревна",     "Кощея в подвале держала. И тренажёры держит."],
  ["Синеглазка",        "Богатырша из Тридевятого царства."],
  ["Златовласка",       "Волосы золотые, мышцы крепкие."],
  ["Царь-девица",       "Полцарства за неё — мало."],
  ["Усоньша",           "Сила трёх мужчин."],
  ["Дуня-богатырка",    "Из крестьянок — в богатырши."],
  // Tier 7 — L63–71: Champion-tier goddesses (PD)
  ["Валькирия",         "Забирает лучших. В зале — ты."],
  ["Афина",             "Шлем, копьё, штанга."],
  ["Артемида",          "Охотится за рекордами. Всегда попадает."],
  ["Беллона",           "Война — её стихия. Зал — арена."],
  ["Нике",              "На каждом подходе — её крылья."],
  ["Энио",              "Хаос на других, порядок на штанге."],
  ["Минерва",           "Мудрость + сила = ты."],
  ["Диана-охотница",    "Стрелы летят прямо. Жим — тоже."],
  ["Юнона",             "Царица богов и зала."],
  // Tier 8 — L72–80: Legendary primordial goddesses (PD)
  ["Гайя",              "Земля носит — ты поднимаешь."],
  ["Рея",               "Мать богов. Штанги её боятся."],
  ["Тефида",            "Океан в твоих венах."],
  ["Селена",            "Луна светит — ты тренируешься."],
  ["Эос",               "Утренние тренировки — твоя стихия."],
  ["Геката",            "Три ипостаси, одна сила."],
  ["Фемида",            "Весы балансируют только её железо."],
  ["Немезида",          "Слабость наказана."],
  ["Мать-Земля",        "Твоё имя будут помнить века."],
];
